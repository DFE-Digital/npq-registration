...
# Change ownership of the application directory and bundle directory
# Rails needs write access to tmp/, log/, and potentially other directories
RUN chown -hR appuser:appgroup /app && \
    chown -hR appuser:appgroup /usr/local/bundle/ && \
    # Create common Rails directories if they don't exist and set proper permissions
    mkdir -p /app/tmp /app/log /app/storage && \
    chown -hR appuser:appgroup /app/tmp /app/log /app/storage
