module Migration
  ...
  private
    ...
    def opposite_users(user)
      user.is_a?(User) ? ecf_users : npq_users
    end

    def ecf_orphan_with_no_applications?(match)
      return unless match.orphaned?
      return unless match.orphan.is_a?(Migration::Ecf::User)
    ...
    def ecf_user_ids_with_npq_applications
      @ecf_user_ids_with_npq_applications ||= Set.new(
        Migration::Ecf::User.includes(participant_identities: :npq_applications)
