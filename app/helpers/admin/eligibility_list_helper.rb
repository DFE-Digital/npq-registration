module Admin::EligibilityListHelper
  def last_bulk_operation_status(bulk_operation)
    if bulk_operation&.finished? && bulk_operation.result.present?
      "Failed processing at #{bulk_operation.finished_at.to_formatted_s(:govuk_short)} - #{bulk_operation.result}"
    elsif bulk_operation&.finished?
