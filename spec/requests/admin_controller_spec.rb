...
RSpec.describe AdminController, type: :request do
  ...
  describe "legacy /npq-separation/admin redirects" do
    it "redirects the namespace root to /admin" do
      get "/npq-separation/admin"
      expect(response).to redirect_to("/admin")
    end

    it "redirects deep links to /admin" do
      get "/npq-separation/admin/applications"
      expect(response).to redirect_to("/admin")
    end
  end
end
