...
RSpec.describe "Participant endpoints", type: :request do
  ...
  describe("show") do
    before { api_get(api_v1_participant_path(123)) }

    specify { expect(response).to(be_method_not_allowed) }
  end

  describe("change_schedule") do
    before { api_put(api_v1_participant_change_schedule_path(123)) }

    specify { expect(response).to(be_method_not_allowed) }
  end

  describe("defer") do
    before { api_put(api_v1_participant_defer_path(123)) }

    specify { expect(response).to(be_method_not_allowed) }
  end

  describe("withdraw") do
    before { api_put(api_v1_participant_withdraw_path(123)) }

    specify { expect(response).to(be_method_not_allowed) }
  end

  describe("resume") do
    before { api_put(api_v1_participant_resume_path(123)) }
