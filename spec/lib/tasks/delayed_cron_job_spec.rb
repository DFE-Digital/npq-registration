...
RSpec.describe "Delayed::Cron::Job tasks" do
  describe "delayed_cron_job:schedule" do
    ...
    context "when there is a deleted delayed cron job" do
      ...
      before do
        stub_const("DeletedCronJob", Class.new(CronJob))
        ...
        deleted_job = Delayed::Job.enqueue ActiveJob::QueueAdapters::DelayedJobAdapter::JobWrapper.new("job_class" => DeletedCronJob.to_s)
        deleted_job.update!(cron: "31 1 * * *")
        stub_const("DeletedCronJob", nil)
      ...
      it "deletes the delayed job for the deleted cron job" do
        run_task
        expect(all_delayed_jobs.count).to eq 2
        expect(cron_delayed_jobs.count).to eq 1
