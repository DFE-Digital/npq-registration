...
RSpec.describe "one_off:clawback_duplicate_delcarations" do
  subject :run_task do
    Rake::Task["one_off:clawback_duplicate_delcarations"].invoke(dry_run)
  end
  ...
  after { Rake::Task["one_off:clawback_duplicate_delcarations"].reenable }

  context "when dry run is false" do
    let(:dry_run) { "false" }

    it "voids the duplicate declarations" do
      run_task
    ...
    it "attaches the awaiting clawback declarations to a statement" do
      run_task
