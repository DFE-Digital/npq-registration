...
RSpec.describe Migration::ReconcileApplications do
  ...
  describe "#duplicated" do
    ...
    context "when there are duplicated matches" do
      let!(:ecf_application) { create(:ecf_npq_application) }
      let!(:npq_application) { create(:application, ecf_id: ecf_application.id) }
      let!(:duplicate_npq_application) { create(:application, ecf_id: ecf_application.id) }
  ...
  describe "#matched" do
    ...
    context "when there are orphans" do
      before { create(:ecf_npq_application) }
  ...
  describe "#orphaned_matches" do
    ...
    context "when an NPQ/ECF application share a course name and user" do
      ...
      let!(:ecf_course) { create(:ecf_npq_course, name: npq_course.name.upcase) }
      let!(:ecf_participant_identity) { create(:ecf_participant_identity, user: ecf_user) }
      let!(:ecf_application) { create(:ecf_npq_application, participant_identity: ecf_participant_identity, npq_course: ecf_course) }

      let!(:npq_user) { create(:user, ecf_id: ecf_user.id) }
      let!(:npq_course) { Course.all.sample }
      let!(:npq_application) { create(:application, user: npq_user, course: npq_course) }
