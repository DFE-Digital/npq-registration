...
RSpec.describe Questionnaires::ChooseYourNpq, type: :model do
  describe "validations" do
    let(:valid_course_identifier) { "npq-early-headship-coaching-offer" }

    it { is_expected.to validate_presence_of(:course_identifier) }

    it "course for course_id must be available to applicant" do
      subject.course_identifier = "npq-additional-support-offer"
      subject.valid?
  ...
  describe "#next_step" do
    ...
    let(:course_identifier) { Course.find_by!(identifier: "npq-leading-behaviour-culture").identifier }
    ...
    context "when changing answers" do
      ...
      context "when changing to something other than headship" do
        let(:course) { Course.find_by!(identifier: "npq-leading-teaching") }
