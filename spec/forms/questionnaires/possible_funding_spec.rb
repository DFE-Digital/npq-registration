...
RSpec.describe Questionnaires::PossibleFunding do
  ...
  describe "#previous_step" do
    ...
    context "when the course is SENCO" do
      before { store["course_identifier"] = "npq-senco" }

      it { is_expected.to be(:senco_start_date) }
    end

    context "when the course is EHCO" do
      before { store["course_identifier"] = "npq-early-headship-coaching-offer" }

      it { is_expected.to be(:ehco_new_headteacher) }
    end

    context "when the course is not NPQLPM, SENCO or EHCO" do
