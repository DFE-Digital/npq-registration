...
RSpec.describe DeclarationDateValidator do
  ...
  describe "#declaration_date" do
    ...
    context "when declaration_date is after the schedule start" do
      let(:schedule_applies_to_date) { declaration_date - 1.day }

      it "has a meaningfull error", :aggregate_failures do
        expect(subject).to be_invalid
        expect(subject.errors.first).to have_attributes(attribute: :declaration_date, type: :declaration_after_schedule_cutoff)
      end
    end
  end
end
