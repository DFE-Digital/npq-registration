...
RSpec.describe RegistrationWizard do
  describe "#current_step" do
    ...
    subject { described_class.new(current_step: "share_provider", store: store, session: session, request: request) }
    ...
    context "when invalid step" do
      subject { described_class.new(current_step: "i_do_not_exist", store: store, session: session, request: request) }
