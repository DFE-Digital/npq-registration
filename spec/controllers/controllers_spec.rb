...
RSpec.describe "choosing the correct controller to inherit from" do
  ...
  describe "sessions with NullUser in" do
    ...
    context "with a NullUser set" do
      before do
        session[:user_id] = 1
        session[RegistrationWizard::STORE_SESSION_KEY] = { current_user: NullUser.new }
      end

      it "clears out the session" do
        get :index
        expect(session[:user_id]).to be_nil
        expect(session[RegistrationWizard::STORE_SESSION_KEY]).to be_nil
      end
    end
    ...
    context "without a NullUser set" do
      before do
        session[:user_id] = user.id
        session[RegistrationWizard::STORE_SESSION_KEY] = { current_user: user }
      end

      it "does not clear out the session" do
        get :index
        expect(session[:user_id]).to eq user.id
        expect(session[RegistrationWizard::STORE_SESSION_KEY][:current_user]).to eq user
      end
    end
  end
end
