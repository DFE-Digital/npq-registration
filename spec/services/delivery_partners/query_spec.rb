...
RSpec.describe DeliveryPartners::Query do
  ...
  describe "#delivery_partners" do
    let!(:delivery_partner_1) { create :delivery_partner, lead_providers: { cohort_21 => lead_provider_1 }, created_at: 2.minutes.ago }
    let!(:delivery_partner_2) { create :delivery_partner, lead_providers: { cohort_22 => lead_provider_2 }, created_at: 1.minute.ago }
    let!(:delivery_partner_an_hour_ago) { create :delivery_partner, lead_providers: { cohort_23 => lead_provider_2 }, created_at: 1.hour.ago }

    it "returns all delivery partners ordered by created_at" do
    ...
    context "when sorting" do
      ...
      context "when sort is +created_at" do
        subject(:query) { described_class.new sort: "+created_at" }
    ...
    context "when filtering by lead_provider" do
      ...
      it "returns delivery partners for the specified lead_provider" do
        expect(query.delivery_partners).to eq([delivery_partner_1])
      end
    end

    context "when filtering by cohort" do
      subject(:query) { described_class.new cohort_start_year: cohort_22.start_year }
