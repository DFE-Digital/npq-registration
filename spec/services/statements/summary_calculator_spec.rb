...
RSpec.describe Statements::SummaryCalculator do
  ...
  describe "#total_payment" do
    ...
    let!(:contract) { create(:contract, course:, statement:, contract_template:) }
  ...
  describe "#total_voided" do
    ...
    context "when there are multiple declarations for the same user" do
      before do
        user = create :user
        create(:declaration, :voided, lead_provider: lead_provider, statement:, application: create(:application, user:))
        create(:declaration, :voided, lead_provider: lead_provider, statement:, application: create(:application, user:))
        create(:declaration, :voided, lead_provider: lead_provider, statement:)
      end
