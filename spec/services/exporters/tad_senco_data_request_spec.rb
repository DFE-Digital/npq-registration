...
RSpec.describe Exporters::TadSencoDataRequest do
  ...
  describe "#call" do
    let(:headers) do
      "Full Name,Email,Application ID,Application ECF ID,User ID,User ECF ID,TRN Verified,Lead Provider ID,Lead Provider ECF ID,Employment Role,Employment Type,Work Setting,Works In School,School URN,Private Childcare Provider URN,Number Of Pupils,Eligible For Funding,Funding Choice,Funded Place,Senco In Role,Senco Start Date,Started Course,Lead Provider Approval Status,Training Status,Headteacher Status,Cohort"
    end
    let(:expected_csv) do
      <<~CSV
        #{headers}
        #{expected_name},#{expected_email},#{application.id},#{application.ecf_id},#{user.id},#{user.ecf_id},#{user.trn_verified},#{application.lead_provider.id},#{application.lead_provider.ecf_id},#{application.employment_role},#{application.employment_type},#{application.work_setting},#{application.works_in_school},#{application&.school&.urn},#{application&.private_childcare_provider&.urn},#{application&.school&.number_of_pupils},#{application.eligible_for_funding},#{application.funding_choice},#{application.funded_place},#{application.senco_in_role},#{application.senco_start_date},#{expected_started},#{application.lead_provider_approval_status},#{application.training_status},#{application.headteacher_status},#{cohort.start_year}
      CSV
    end
