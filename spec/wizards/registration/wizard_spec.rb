...
RSpec.describe Registration::Wizard do
  ...
  it { is_expected.to have_next_step(:check_answers).from(:course_start_date).when(course_start_cohort: nil) }
  it { is_expected.to have_next_step(:check_answers).from(:course_start_date).when(course_start_cohort: "2026b") }
  it { is_expected.to have_next_step(:cannot_register_yet).from(:course_start_date).when(course_start_cohort: "2026a") }
  it { is_expected.to have_next_step(:applications_list).from(:check_answers) }

  describe "paths to end of journey" do
    before { allow(Cohort).to receive(:find_by).and_return cohort }
    ...
    context "when 2026b at course_start_cohort" do
      let(:state) { { started: true, course_start_cohort: "2026b" } }

      it { is_expected.to have_flow_path(%i[start course_start_date check_answers]) }
      it { is_expected.to have_valid_path(%i[start course_start_date check_answers]) }
    end

    context "when 2026a at course_start_cohort" do
      let(:state) { { started: true, course_start_cohort: "2026a" } }
