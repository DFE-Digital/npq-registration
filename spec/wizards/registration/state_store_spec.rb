...
RSpec.describe Registration::StateStore do
  ...
  it { is_expected.to have_step_attribute(:course_start_cohort).with_value("2026b") }

  describe "#[]" do
    context "with symbol key" do
      subject { store[:course_start_cohort] }

      it { is_expected.to eq "2026b" }
    end
  ...
  describe "#not_starting_in_current_cohort?" do
    ...
    context "with value set to no" do
      let(:state) { { course_start_cohort: "2026b" } }

      it { is_expected.to be false }
    end

    context "with value set to yes" do
      let(:state) { { course_start_cohort: "2026a" } }
