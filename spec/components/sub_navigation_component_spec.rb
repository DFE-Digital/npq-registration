...
class TestSubNavigationStructure < NavigationStructure
  def structure
    {
      Node.new(
        name: "First",
...
RSpec.describe SubNavigationComponent, type: :component do
  ...
  subject do
    SubNavigationComponent.new(current_path, structure: structure.sub_structure(current_path, default_to_first_section:), heading:)
  end
  ...
  context "when an unrecognised section is provided" do
    ...
    it "raises an error" do
      expect { render_inline(subject) }.to raise_error(NavigationStructure::SectionNotFoundError)
    end
