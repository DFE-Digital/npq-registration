...
namespace :lead_providers do
  ...
  task :seed_statements_and_applications, %i[lead_provider_name number_of_participants cohort_name] => :environment do |_t, args|
    ...
    cohort = Cohort.find_by(name: args[:cohort_name])
    raise "Cohort not found: #{args[:cohort_name]}" if args[:cohort_name] && !cohort
