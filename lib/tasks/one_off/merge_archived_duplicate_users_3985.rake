...
# into the not archived user with the same verified TRN.
...
namespace :one_off do
  ...
  task :merge_archived_duplicate_users, %i[dry_run] => :versioned_environment do |_task, args|
    ...
    archived_duplicate_users.find_each do |user_to_merge|
      users_to_keep = User.not_archived.with_trn(user_to_merge.trn).to_a

      if users_to_keep.size != 1
        logger.info "Skipping user #{user_to_merge.ecf_id}: found #{users_to_keep.size} not archived users with TRN #{user_to_merge.trn}"
        skipped_count += 1
        next
      end
