namespace :one_off do
  ...
  task :fix_locked_users, %i[dry_run log_pii] => :versioned_environment do |_t, args|
    ...
    # users 1996 and 2156 are archived, but should not be unarchived because there is already a later user with the same email
    # but they are from before 13/7/26
    ...
    date_range = (Date.parse("2026-06-14")..) # rubocop:disable Style/RedundantParentheses

    user_list.each do |user_id|
      user = User.where(id: user_id).first

      trn_versions = user.versions.where(whodunnit:, created_at: date_range).select { |v| v.object_changes.keys.include?("trn") }
      trn_update_ids << user_id if trn_versions.any?

      archived_versions = user.versions.where(whodunnit:, created_at: date_range).select { |v| v.object_changes.keys.include?("archived_at") }
    ...
    logger.info ""
    logger.info "extra users with archived_at changes: #{archived_update_ids.uniq - user_list_b.uniq}"
    logger.info ""
    ...
    ActiveRecord::Base.transaction do
      ...
      user_list_b.each do |user_id|
        ...
        unarchived_versions = user.versions.where(whodunnit:, created_at: date_range).select { |v| v.object_changes.keys.include?("archived_at") }
      ...
      user_list_a.each do |user_id|
        user = User.find(user_id)
        first_trn_version = user.versions.where(whodunnit:, created_at: date_range).select { |v| v.object_changes.keys.include?("trn") }.first
      ...
      applications.each do |application_id|
        ...
        user_id_versions = application.versions.where(whodunnit:, created_at: date_range).select { |v| v.object_changes.keys.include?("user_id") }
        ...
        application.update!(user_id: original_user_id)
        ...
        if last_participant_id_change
          ...
          raise "unexpected participant_id_change" if ParticipantIdChange.where(from_participant_id: application.user.ecf_id).exists?
        else
